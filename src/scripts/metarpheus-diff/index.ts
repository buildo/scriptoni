...
const apiExitCode =
  structuredPatch("", "", api, apiNew, "", "").hunks.length === 0 ? 0 : 1;
const apiOutput = colorizePatch(
  createTwoFilesPatch("current", "new", api, apiNew, "", "")
);
...
const modelExitCode =
  structuredPatch("", "", model, modelNew, "", "").hunks.length === 0 ? 0 : 1;
const modelOutput = colorizePatch(
  createTwoFilesPatch("current", "new", model, modelNew, "", "")
);
